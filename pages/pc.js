                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="border-b border-gray-200 text-sm text-gray-600 font-medium bg-white">
                            ...
                            <th class="p-4 bg-indigo-50/50 w-48">代理节点分配</th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr v-for="pc in paginatedPcList" :key="pc.mac" class="border-b border-gray-100 hover:bg-gray-50 transition text-sm bg-white">
                            ...
                            <td class="p-4 bg-indigo-50/20">
                                <select v-if="pc.ip" v-model="pc.singboxProxy" @change="changeSingboxProxy(pc)" :disabled="pc.rosTable !== 'proxy'" class="border border-gray-200 rounded px-2 py-1.5 text-xs bg-white focus:outline-none focus:border-indigo-500 cursor-pointer w-full shadow-sm hover:border-indigo-300 transition disabled:opacity-50 disabled:bg-gray-100 disabled:cursor-not-allowed">
    ...
    setup() {
            ...
            try {
                ...
                if (rosRes.status === 'success') {
                    ...
                    pcList.value = rawPcs.map(pc => {
                        ...
                        return { ...pc, singboxProxy: assignedSingbox, rosTable: assignedRosTable };
                    });
                } else {
        ...
        const changeRosRouting = async (pc) => {
            ...
            if (pc.rosTable !== 'proxy' && pc.singboxProxy !== 'block') {
                ...
                await fetch('api/action', {
                    method: 'POST', headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({ action: 'change_proxy', payload: { tag: 'block', ip: pc.ip } })
                });
            }
            ...
            try {
                const res = await fetch('api/ros', {
                    ...
                    body: JSON.stringify({
                        ...
                        payload,
                        data: { cidr: pc.ip, table: pc.rosTable }
                    })
                }).then(r => r.json());
        ...
        const changeSingboxProxy = async (pc) => {
            ...
            try {
                await fetch('api/action', {
                    method: 'POST', headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({ action: 'change_proxy', payload: { tag: pc.singboxProxy, ip: pc.ip } })
                }).then(r => r.json());
            } catch (e) {
        ...
        return {
            ...
            fetchData, changeRosRouting, changeSingboxProxy
        };
    }
};
