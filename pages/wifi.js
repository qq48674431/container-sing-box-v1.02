export default {
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="border-b border-gray-200 text-sm text-gray-600 font-medium bg-white">
                            ...
                            <th class="p-4 bg-indigo-50/50 w-40">代理节点</th>
                    ...
                    <tbody>
                        <tr v-if="isLoading && wifiList.length === 0" class="border-b border-gray-100 bg-gray-50/30">
                            <td colspan="6" class="p-8 text-center text-gray-400">正在与 RouterOS 通信获取路由结构...</td>
                        </tr>
                        ...
                        <tr v-for="wifi in paginatedWifiList" :key="wifi.name" class="border-b border-gray-100 hover:bg-gray-50 transition text-sm bg-white">
                            ...
                            <td class="p-4 bg-indigo-50/20">
                                <select v-if="wifi.ipAddress && wifi.ipAddress !== '未分配网段'" v-model="wifi.singboxProxy" @change="changeSingboxProxy(wifi)" :disabled="wifi.rosTable !== 'proxy'" class="border border-gray-200 rounded px-2 py-1.5 text-xs bg-white focus:outline-none focus:border-indigo-500 cursor-pointer w-full shadow-sm disabled:opacity-50 disabled:bg-gray-100">
    ...
    setup() {
        ...
        const editForm = ref({ ssid: '', macAddress: '' });
            ...
            try {
                ...
                if (res.status === 'success') {
                    ...
                    wifiList.value = rawWifis.map(wifi => {
                        ...
                        return { ...wifi, networkCidr, singboxProxy: assignedSingbox, rosTable: assignedRosTable };
                    });
                } else {
        ...
        const changeRosRouting = async (wifi) => {
            ...
            if (wifi.rosTable !== 'proxy' && wifi.singboxProxy !== 'block') {
                ...
                await fetch('api/action', {
                    method: 'POST', headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({ action: 'change_proxy', payload: { tag: 'block', ip: wifi.networkCidr } })
                });
            }
            ...
            try {
                const res = await fetch('api/ros', {
                    ...
                    body: JSON.stringify({
                        ...
                        payload,
                        data: { cidr: wifi.networkCidr, table: wifi.rosTable }
                    })
                }).then(r => r.json());
        ...
        const changeSingboxProxy = async (wifi) => {
            ...
            try {
                await fetch('api/action', {
                    method: 'POST', headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({ action: 'change_proxy', payload: { tag: wifi.singboxProxy, ip: wifi.networkCidr } })
                }).then(r => r.json());
            } catch (e) {
                console.error("代理修改失败");
            }
        };
        ...
        return {
            ...
            changeRosRouting, changeSingboxProxy, toggleStatus,
